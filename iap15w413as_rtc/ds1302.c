...
static u8 hex2bcd(u8 hex)
{
	return (hex / 10 << 4) + (hex % 10);
}

static u8 bcd2hex(u8 bcd)
{
...
void send_time(void)
{
	u8 year = 0, mon = 0, day = 0, hour = 0, min = 0, sec = 0, week = 0;
	
	ds1302_get_date(&year, &mon, &day, &hour, &min, &sec, &week);
